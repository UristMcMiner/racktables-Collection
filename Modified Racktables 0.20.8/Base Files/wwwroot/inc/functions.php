		...
		for ($locidx = 0; $locidx < 3; $locidx++)
		{
			...
			if ($template[$template_idx][$locidx])
			{
				...
				(
					...
					($rackData[$startRow - $height][$locidx]['state'] != 'T' & $rackData[$startRow - $height][$locidx]['state'] != 'M' )
				)
					break 2;
				if ($object_id == 0)
...
function applyObjectMountMask (&$rackData, $object_id)
{
	for ($unit_no = $rackData['height']; $unit_no > 0; $unit_no--)
		for ($locidx = 0; $locidx < 3; $locidx++)
			...
			{
				case 'M'://added
					$rackData[$unit_no][$locidx]['enabled'] = ($rackData[$unit_no][$locidx]['object_id'] == $object_id);//added
					break;//added
				case 'F':
...
{
	$counter = array ('A' => 0, 'U' => 0, 'T' => 0, 'W' => 0, 'F' => 0, 'M' => 0);
	for ($unit_no = $data['height']; $unit_no > 0; $unit_no--)
		for ($locidx = 0; $locidx < 3; $locidx++)
...
//FUNCTIONS ADDED BY FLORIAN BAIER
// |
// v
...
function FilterNames($r_0,$keys,$r_mnt){
if(isset($_POST['prob'])){
	for($a=0;$a<count($keys);$a++){
		if($r_0[$a][2]!='yes')unset($r_0[$a]);	
 
	}}
$r_mnt=IndiceArrayByID($r_mnt);
$mkeys=array_keys($r_mnt);
$keys=array_keys($r_0);
if(isset($_POST['mnt'])){
	for($a=0;$a<count($keys);$a++){
		if($r_mnt[$r_0[$keys[$a]][0]][1]!='yes')unset($r_0[$keys[$a]]);
}}
return $r_0;
}
 
function IndiceArrayByID($arr){
$keys=array_keys($arr);
for($a=0;$a<count($keys);$a++){
	$ind=$keys[$a];
	$id=$arr[$a][0];
...
function getLinkOverPatchPanel($remote_object_id){
	echo $remote_object_id;
}
 
function renderHoverTextBegin($remote_object_id){
 
}
 
function renderHoverTextFinish($remote_object_id){
 
}
 
function updateMGMT(){
	$fetch0 = usePreparedSelectBlade('SELECT object_id, ip, name FROM IPv4Allocation');
	$res = $fetch0->fetchAll();
	$objects=checkForMGMT();
	$lastip=getConfigVar('LAST_IP_RAC');
	foreach($objects as $obj){
		if(getIndiceByID($res, $obj[0])===FALSE){
		usePreparedExecuteBlade('INSERT IGNORE INTO IPv4Allocation(object_id, ip, name) VALUES ('.$obj[0].','.$lastip++.',"'.$obj[1].'")');
		usePreparedExecuteBlade('UPDATE object_state_information SET has_rac="no" WHERE id='.$obj[0]);
		//usePreparedExecuteBlade('UPDATE AttributeValue SET string_value="No RAC Available" WHERE (object_id='.$obj[0].' AND attr_id=10004');
		}
	}
 
	foreach($res as $row){
		if($row[1]>=16842753 && $row[1]<=16846846){
			usePreparedExecuteBlade('UPDATE object_state_information SET has_rac="no" WHERE id='.$row[0]);
			//usePreparedExecuteBlade('UPDATE AttributeValue SET string_value="No RAC Available" WHERE (object_id='.$row[0].' AND attr_id=10004)');
		}
		if(!($row[1]>=16842753 & $row[1]<=16846846)){
			$fetch1 = usePreparedSelectBlade('SELECT id, objtype_id FROM Object WHERE id='.$row[0]);
			$res2 = $fetch1 -> fetchAll();
			$ip_bin = ip4_int2bin($row[1]);
			$raddr = ip4_format($ip_bin);
 
			if(inDeviceList($res2[0][1])===TRUE){
			usePreparedExecuteBlade('UPDATE object_state_information SET has_rac="yes" WHERE id='.$row[0]);
			$wert = usePreparedExecuteBlade('UPDATE AttributeValue SET string_value="'.$raddr.'" WHERE (object_id='.$row[0].' AND attr_id=10004)');
			if ($wert == 0){
				$fetch1 = usePreparedSelectBlade('SELECT id, objtype_id FROM Object WHERE id='.$row[0]);
				$res2 = $fetch1 -> fetchAll();
				usePreparedExecuteBlade('INSERT IGNORE INTO AttributeValue(object_id, object_tid, attr_id, string_value, uint_value, float_value) VALUES ('.$res2[0][0].','.$res2[0][1].',10004,"'.$raddr.'",NULL,NULL)');
			}
			}					
		}
	}
	$lastip++;
	usePreparedExecuteBlade('UPDATE Config SET varvalue='.$lastip.' WHERE varname="LAST_IP_RAC"');
}
 
function checkForMGMT(){
	$result = usePreparedSelectBlade('SELECT name, object_id FROM Port');
	$port_row = $result->fetchAll();
	$a=0;
	$arr_res = Array();
	foreach ($port_row as $row){
	if(isMGMT($row[0])===TRUE){
		$arr_res[$a][0]=$row[1];
		$arr_res[$a][1]=$row[0];
		$a++;
	}
...
function isMGMT($name){
	$MGMT = Array ('C0P0_MGMT','C0_MGMT','C0_MGMT1','C0_MGMT2','C0P1_MGMT','C1_MGMT','C1_MGMT1','C1_MGMT2','C1P0_MGMT','iDRAC','iLO','iLO2','MGMT');
	$keys = array_keys($MGMT);
	for ($a=0; $a<count($keys);$a++){
		$ind_m = $keys[$a];
		$MGMT[$ind_m] = strtolower($MGMT[$ind_m]);
		$name = strtolower($name);
		if($name==$MGMT[$ind_m])RETURN TRUE;
	}	
	RETURN FALSE;
}	
 
function getIndiceByID($arr, $id){
	foreach($arr as $row){
		if(isMGMT($row[2])&&($row[0]==$id))return $id;
	}
	return FALSE;
}
 
function inDeviceList($type_id){
	$device_list = array(4);
	for($a=0;$a<count($device_list);$a++){
		if($type_id==$device_list[$a])return TRUE;
	}
	RETURN FALSE;
}
?>
