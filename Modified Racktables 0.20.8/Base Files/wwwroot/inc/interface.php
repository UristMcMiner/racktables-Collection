...
function renderRack ($rack_id, $hl_obj_id = 0)
{
	...
	for ($i = $rackData['height']; $i > 0; $i--)
	{
		...
		for ($locidx = 0; $locidx < 3; $locidx++)
		{
			...
			{
				case 'M'://added
					printObjectDetailsForRenderRack ($rackData[$i][$locidx]['object_id'], $hl_obj_id);//added
					break;//added
				case 'T':
...
function renderEditObjectForm()
{
	...
	if ($object['has_problems'] == 'yes')
		echo ' checked';
	echo "></td></tr>\n";
	echo "<br><tr><td>&nbsp;</td><th class=tdright>To Mount:</th><td class=tdleft><input type=checkbox name=object_to_mount";//to_mount //added
	if($object['to_mount']=='yes'){//added 																						 //to_mount	
	echo ' checked';//added																										 //to_mount
	}																														 //to_mount
	echo "></td></tr>\n";//added
	echo "<tr><td>&nbsp;</td><th class=tdright>Actions:</th><td class=tdleft>";
...
function renderObject ($object_id)
{
	...
	if ($info['has_problems'] == 'yes')
		$summary[] = array ('<tr><td colspan=2 class=msg_error>Has problems</td></tr>');
	if ($info['to_mount'] == 'yes')//added
		$summary[] = array ('<tr><td colspan=2 class=msg_tomount>Object is not mounted yet</td></tr>');//added
	if ($info['has_rac'] == 'no' && inDeviceList($info['objtype_id']))//added
		$summary[] = array ('<tr><td colspan=2 class=msg_error>Object is missing Management Address</td></tr>');//added
	...
	elseif (! ($cellfilter['is_empty'] && renderEmptyResults ($cellfilter, 'objects', $objects_count)))
	{
		...
		if (! renderEmptyResults ($cellfilter, 'objects', count($objects)))
		{
			...
			foreach ($objects as $obj)
			{
				...
				$mount = ($obj['to_mount'] == 'yes') ? 'to_mount' : '';//added
