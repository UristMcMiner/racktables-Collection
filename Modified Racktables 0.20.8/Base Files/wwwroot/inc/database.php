...
(
	...
	(
		...
		(
			...
			'to_mount' => '(SELECT to_mount FROM object_state_information WHERE id = RackObject.id)',//added
			'has_rac'  => '(SELECT has_rac FROM object_state_information WHERE id = RackObject.id)',//added
			'comment' => 'comment',
...
function commitAddObject ($new_name, $new_label, $new_type_id, $new_asset_no, $taglist = array())
{
	...
	//ADD ENTRY TO OBJECT_STATE_INFORMATION                    -> Beim Hinzufügen Eintrag in object_state_information anlegen
	usePreparedInsertBlade
	(
		'object_state_information',
		array
		(
		'id' => $object_id		
		)
...
function commitUpdateObject ($object_id, $new_name, $new_label, $new_has_problems, $new_asset_no, $new_comment)
{
	...
	usePreparedUpdateBlade//<added>
	(
	    'object_state_information',
		array
		(
		'to_mount' => $new_to_mount
		),
		array
		(
			'id' => $object_id
	...
	//IMPLEMENTIERUNG DES NEUEN STATUS
	if($new_to_mount == 'yes')usePreparedUpdateBlade
								(
								'RackSpace',
								array('state' => 'M'),
								array('object_id' => $object_id)
								);
	if($new_to_mount == 'no')usePreparedUpdateBlade
								(
								'RackSpace',
								array('state' => 'T'),
								array('object_id' => $object_id)
