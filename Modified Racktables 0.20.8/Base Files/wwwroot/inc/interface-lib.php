...
function renderEntitySummary ($cell, $title, $values = array())
{
	...
	foreach ($values as $name => $value)
	{
		...
		if ($name == 'RemoteAccess:'){	//added
			echo "<tr><th width='50%' class='$class'>$name</th><td class=tdleft><a href=http://$value target=_blank>Link</a></td></tr>";//added
			$RACFlag=TRUE;//added
		}//added
		if ($name == 'tags:')
		{
			...
			if ($RACFlag === FALSE)echo "<tr><th width='50%' class='$class'>$name</th><td class=tdleft>$value</td></tr>";}
	}
	echo "</table>\n";
	finishPortlet();
}
